 ...
 */
export async function insertRecordTypeArray(faker: Faker, prisma: PrismaClient, count: number = 5) {
  let newArr: Prisma.RecordTypeUncheckedCreateInput[] = [];
  ...
  while (newArr.length < count) {
    ...
    let d: Prisma.RecordTypeUncheckedCreateInput = getOneRecordType(faker, candidateIds, );
    newArr.push(d);
  }
  await prisma.recordType.createMany({ data: newArr });
}
 ...
 * @returns An object containing fake data for a single role record.
 */
function getOneRecordType(faker: Faker, candidateIds: { [modelName: string]: string[] }, ): Prisma.RecordTypeUncheckedCreateInput {
  return {
    breakTypeId: faker.datatype.uuid(),
    breakName:  faker.word.noun(),
    breakDescription:  faker.word.noun(),
  };
}
