 ...
 */
export async function insertAttendanceArray(faker: Faker, prisma: PrismaClient, count: number = 5) {
  let newArr: Prisma.AttendanceUncheckedCreateInput[] = [];
  ...
  while (newArr.length < count) {
    ...
    let d: Prisma.AttendanceUncheckedCreateInput = getOneAttendance(faker, candidateIds, );
    newArr.push(d);
  }
  await prisma.attendance.createMany({ data: newArr });
}
 ...
 * @returns An object containing fake data for a single role record.
 */
function getOneAttendance(faker: Faker, candidateIds: { [modelName: string]: string[] }, ): Prisma.AttendanceUncheckedCreateInput {
  return {
    attendanceId: faker.datatype.uuid(),
    employeeId: randomValueFromArray(candidateIds['employeeId'], false),
    checkin:  faker.datatype.bigInt(),
    checkout:  faker.datatype.bigInt(),
    recordTypeId: randomValueFromArray(candidateIds['recordTypeId'], false),
    taskId: randomValueFromArray(candidateIds['taskId'], false),
  };
}
...
async function findCandidateIds(
  ...
  let employeeIdArr = await prisma.employee.findMany({
    select: { employeeId: true },
  });
  candidateIds['employeeId'] = employeeIdArr.map((item) => item.employeeId);

  let recordTypeIdArr = await prisma.recordType.findMany({
    select: { breakTypeId: true },
  });
  candidateIds['recordTypeId'] = recordTypeIdArr.map((item) => item.breakTypeId);

  let taskIdArr = await prisma.task.findMany({
    select: { taskId: true },
  });
  candidateIds['taskId'] = taskIdArr.map((item) => item.taskId);
}
