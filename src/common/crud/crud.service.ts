...
import { modelPrimaryKeys, snakeCaseToCamelCase, unixTimestamp } from '../common.helper';
...
export class CrudService {
    ...
    try {
      ...
      if (exception.code == 'P2002') {
        if (exception.meta.target == 'PRIMARY') {
          throw new ConflictException(`${snakeCaseToCamelCase(modelPrimaryKeys[model])} is already used`);
        }
      }
      throw exception;
    }
  ...
  async upsert(model: string, select: any, where: any, updateData: any, createData: any) {
    try {
      // @ts-ignore
      ...
      if (exception.code == 'P2002') {
        if (exception.meta.target == 'PRIMARY') {
          throw new ConflictException(`${modelPrimaryKeys[model]} is already used`);
        }
      }
      throw exception;
    }
