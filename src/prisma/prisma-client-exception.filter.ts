...
export class PrismaClientExceptionFilter extends BaseExceptionFilter {
  ...
  catchClientKnownRequestError(
    ...
    switch (exception.code) {
      case 'P2000':
        colName = snakeCaseToCamelCase(exception.meta.column_name as string);
        message = `update: ${colName} is too long!`;
        break;

      case 'P2002':
        const col =
          exception.meta.target == 'PRIMARY'
            ? modelPrimaryKeys[getModel(exception.message)]
            : (exception.meta.target as string).split('_')[1];
        colName = snakeCaseToCamelCase(col);
        message = `${colName} is already used`;
        break;

      case 'P2003':
        colName = snakeCaseToCamelCase(exception.meta.field_name as string);
      ...
      case 'P2022':
        colName = snakeCaseToCamelCase(exception.meta.column as string);
